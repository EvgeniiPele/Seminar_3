...
SELECT * FROM shops, cats
WHERE shops.id = cats.shops_id AND cats.name = "Murzik";

-- Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT shops.shopname, pet.name
FROM shops
JOIN
(SELECT * FROM cats
WHERE name != "Murzik" AND name != "Zuza") AS pet
ON shops.id = pet.shops_id;
...
SELECT analysis.an_name, analysis.an_price, test.ord_datetime FROM analysis
JOIN
(SELECT * FROM Orders
WHERE Orders.ord_datetime BETWEEN '2020.02.05' AND '2020.02.13') AS test
ON analysis.an_id = test.ord_an;